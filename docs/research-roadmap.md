...
10 seed × 12 event-layout、120 cellsのstdlib-only offline replayを、実装前に固定したschema/configとengineering／performanceの二段gateで実施する計画です。canonical JSON identity（UTF-8、sort keys、compact separators、末尾改行なし）によるschema/config pinとraw-byte監査値、expanded accounting windowのmode/test内収容、seed-cluster block bootstrap、8個の完全修飾target signal availability、stopped／cooldown faultのsynthetic stress testとしての位置付けを含むseed、layout、detector parameter、指標、promotion閾値、安全境界は [`anomaly-multiseed-evaluation-plan.md`](anomaly-multiseed-evaluation-plan.md) に記録します。Savepoint A validatorとSavepoint B deterministic runner／fixed configおよび各cellのcaptured dataからのquality gate・evaluator semantics・limitationsの純粋再計算・canonical exact比較は実装済みです。v0.1の正式120-cell artifactは生成後監査でsummary integrity bypassが見つかったためREJECT evidenceとして保全し、詳細を[`docs/results/anomaly-multiseed-v01-integrity-audit-2026-09-05.md`](results/anomaly-multiseed-v01-integrity-audit-2026-09-05.md)に記録しました。修正後の正式run、bootstrap、performance evaluation、model昇格は未実施で、再実行にはsummary integrity fixを理由とする[`v0.2 preregistration`](anomaly-multiseed-evaluation-plan-v0.2.md)、別matrix_id、別output_rootが必要です。
