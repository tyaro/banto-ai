...
次の受入境界を実行前に固定する4つのcontrolled scenario（control、target fault、target quality、covariate quality）は[`docs/toto2-controlled-scenarios.md`](toto2-controlled-scenarios.md)を参照してください。ここでは generator／quality／benchmark／Toto mask の静的契約と fake backend integration testだけを追加し、real model、matrix、result artifactは作成していません。truth-unavailableは別の inconclusive／no-rank track とし、stale_valueはsource age／transport freshnessの証拠と解釈しません。
