...
この段階では120-cell run、結果artifact生成、性能達成を完了したとは扱わない。Savepoint Aのschema/config/layout validator、Bのdeterministic matrix runner、Cのfake/unit tests、Dのclean run、Eの独立監査・結果docを順に実施する。Savepoint Bのrunnerとfake/unit testsは実装済みであり、各cellの初期captured observations／events／splitと固定configからのevaluator semantics純粋再計算、canonical exact比較、cell完了時のreplay context破棄も実装済みだが、実120-cell runと結果artifact生成は未実施である。検証の追加計算量・時間は概ね評価1回分相当である。再実行または計画変更は新version・新preregistrationとし、TimesFM／Chronos／Toto／TSPulseのmodel比較、customer data、control／Banto Hub writeは別工程・別計画とする。

Savepoint Aのpure validator、strict schema、固定matrix config、CLI、回帰testは実装済みである。validatorはcanonical JSON identity（`utf-8-json-sort-keys-compact-no-trailing-newline-v1`）でmatrix config、matrix schema、base generator config、base generator schemaをpinし、開始時と完了直前の再読込差分をfail closedにする。raw-byte SHA-256は監査用provenanceとして別記録する。validatorの成功はconfiguration validのみを示し、summaryも`run_status=not_run`、`performance_status=not_evaluated`であり、run完了、performance gate通過、dataset／result artifact生成を示さない。

Savepoint Bのdeterministic 120-cell runner、strict result schema、fake-based regression test、CLIは実装済みである。seed-major × layout_index順のcell展開、入力snapshot／clean revision境界、event inventory、dataset／evaluation hash inventory、atomic non-overwrite aggregate、通常cell failureの継続、global fail-closed、明示recovery quarantineを実装した。各cellのevaluation検証では、初期captured manifest／generator config／observations／events／split／summary／fingerprintとcaptured hash inventoryからquality gateをpure再計算し、evaluator provenanceのquality gate（status／counts／checks）とexact比較する。同じcaptured inputsと固定generator／evaluator configからprofiles／scores／episodes／incidents／accounting／metrics／exclusions／status／row_counts／limitationsを純粋再計算し、canonical exact比較する。さらに、pure replayのexpected semantic payloadから安定したsummary bytesを生成し、captured `summary.md` とbyte exact比較する。任意summaryと更新済みmarkerの1-cell攻撃は対象cell failure、残り119 cell継続、engineering failとなるfake regressionで固定した。追加の検証計算量・時間は概ね評価1回分相当で、replay contextはcell完了時に破棄しpublication／runtime snapshotへ保持しない。v0.1の正式120-cell artifactは旧summary integrity bypassが判明したためREJECT evidenceとして保全しており、修正後の再実行には別matrix_id／output_rootを持つ[`v0.2 preregistration`](anomaly-multiseed-evaluation-plan-v0.2.md)が必要である。正式再run、bootstrap、performance evaluation、独立analysisは未実施である。
standalone analysisの実装は [`anomaly-multiseed-evaluation-plan-v0.2.md`](anomaly-multiseed-evaluation-plan-v0.2.md) に固定したanalysis config/result schema、seed-cluster bootstrap、ratio-of-sums slice集計、read-only matrix verifier、別output root publisherを対象とする。config validationは`run_status=not_run`、`performance_status=not_evaluated`を返し、formal v0.2 artifactの生成・読取り・analysis・promotionは未実施である。

2026-09-06追記: 上記は各savepoint／freeze時点の記録である。その後、formal v0.2 replayとstandalone analysisに続き、failure diagnostics D2-Bもclean replay commit `cecc2a0fc057c7d5f43be07a53c88ce97595c1ab`で正式公開し、独立read-only監査を完了した。diagnosticsはengineering `pass`、performance `not_evaluated`、exploratory only／promotion不可であり、結果は[`docs/results/anomaly-multiseed-v02-failure-diagnostics-2026-09-06.md`](results/anomaly-multiseed-v02-failure-diagnostics-2026-09-06.md)に記録する。これはTimesFM3の性能結果ではなく、v0.3候補の採否も決定しない。

2026-09-06 v0.3計画追記: 次工程の[`event-aware anomaly v0.3実装前preregistration`](anomaly-multiseed-evaluation-plan-v0.3.md)は、旧正式結果を保持したまま、新seed・causal support・差分対照/phase水準/多変量条件付き残差の有限3候補・paired品質層・共通母数・cluster CI・固定昇格判定を定める。S0 plan freezeのみを今回の範囲とし、config/schema/pure validator、scoring/tests、runner、dry/smoke、formal holdout、独立analysis/audit、result docは別savepointとする。v0.3の実装・run・artifact・性能達成は未実施で、TimesFM residualとcommissioning製品化は別研究計画である。
