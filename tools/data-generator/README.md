...
generatorはmotor／conveyorの複数equipment、motor current、motor temperature、conveyor speed、load proxy、vibration feature、operating mode／recipe stepを扱います。regimeは `stopped`、`startup`、`low_speed`、`nominal`、`high_load`、`cooldown`、eventは `sensor_drift`、`spike`、`dropout`、`overheating_trend`、`jam_or_slip`、`stuck_value`、`stale_value` です。`stuck_value`は値固定・quality `ok`、`stale_value`は値固定・quality `stale`、`dropout`はvalue null・quality `missing`として出力します。event intervalは `[start,end)` で、無効eventは観測へ適用せずground-truthにも出力しません。
