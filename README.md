...
controlled evaluation の受入境界は [`docs/toto2-controlled-scenarios.md`](docs/toto2-controlled-scenarios.md) に追加定義しました。control、target fault、target quality、covariate quality の4 trackを、seed 17／29／42／73／101、horizon 15／30、context 64／120、test origin 384で paired comparisonします。cross-matrix acceptance analyzer の source、config/schema、fake artifact testも追加済みですが、real controlled resultsは未実行です。これはreal modelの結果やartifact、Phase 2完了を追加するものではありません。実行順は4 matrix → analyzerで、コマンドは [`tools/toto2/README.md`](tools/toto2/README.md) に記載しています。
